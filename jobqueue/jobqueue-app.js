...
var script_cwd  = interpolate(config('job_queue').script_wd, {
    app:{root: path.resolve(__dirname)},
    env:process.env
});
var script_path = path.resolve(script_cwd, config('job_queue').script_path);
...
jqapp.prototype = {
    ...
    run_main_loop : function(callback){
        ...
        var self = this;
        var loop_while_more_jobs = this.options.loop_while_more_jobs || config('job_queue').loop_while_more_jobs;
        var loop_delay           = this.options.loop_delay           || config('job_queue').loop_delay          ;
        var rerun_interval       = this.options.rerun_interval       || config('job_queue').rerun_interval      ;
    ...
    cleanup_other_queues: function(){
        var qtimeout = this.options.heartbeat_timeout || config('job_queue').heartbeat_timeout;
        var jtimeout = this.options.job_inactivity_timeout || config('job_queue').job_inactivity_timeout;
        var callback = arguments[arguments.length-1];
        model.job_queues.cleanup_stuck_queues(qtimeout, jtimeout, callback);
    },
    
    get_max_concurrency : function(){
        var concurrency = this.options.concurrency || config('job_queue').concurrency;
    ...
    'get://stats-format' : function(req, res, next){
	var dats = this.get_stats();
        res.send("<html><head></head>"+
		"<body><table>"+
		"<tr><td>queue</td><td> "+dats.queue+"</td></tr>" +
		"<tr><td>iteration</td><td> "+dats.iteration+"</td></tr>" +
		"<tr><td>last_updated</td><td> "+dats.last_updated+"</td></tr>" +
		"<tr><td>state</td><td> "+dats.state+"</td></tr>" +
		"<tr><td>options</td><td> "+JSON.stringify(dats.options)+"</td></tr>" +
		"<tr><td>waiting</td><td> "+dats.waiting+"</td></tr>" +
		"<tr><td>last_enqueued</td><td> "+dats.last_enqueued+"</td></tr>" +
		"<tr><td>max_concurrent</td><td> "+dats.max_concurrent+"</td></tr>" +
		"</table></body></html>");
    },   
