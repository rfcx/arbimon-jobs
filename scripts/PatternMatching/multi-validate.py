...
for i in range(len(validation_data)):
    ...
    if retVal:
        ...
        with closing(db.cursor()) as cursor:    
            ...
            db.commit()        
        rows.append(','.join([r['name'],str(row['totaln']),str(row['pos_n']),str(row['neg_n']),str(row['k_folds']),str(row['accuracy']),str(row['precision']),str(row['sensitivity']),str(row['specificity']),str(row1['time'])]))
        row = None
        row1 = None
    else:
        print 'job failed'
...
print ','.join(['species','total_n','pos','neg','k','accuracy','precision','sensitivity','specificity','exec_time'])
for r in rows:
    print r
